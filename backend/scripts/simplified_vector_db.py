#!/usr/bin/env python3
"""
Script to create a simplified vector database for RAG capabilities.
This script processes the collected eCFR data and creates embeddings for semantic search
using a more lightweight approach with fewer dependencies.
"""
...
# Configure logging with more verbose output and direct to stdout
logging.basicConfig(
    level=logging.INFO,
...
print("Starting simplified vector database creation script")
logger.info("Starting simplified vector database creation script")
...
EMBEDDING_DIM = 768  # Dimension of embeddings (we'll use a simple TF-IDF approach)
...
def load_data():
    ...
    if os.path.exists(agencies_file):
        ...
        print(f"Loaded agencies data from {agencies_file}")
    else:
        logger.warning(f"Agencies data file not found: {agencies_file}")
        print(f"Agencies data file not found: {agencies_file}")
    ...
    if os.path.exists(titles_file):
        ...
        print(f"Loaded titles data from {titles_file}")
    else:
        logger.warning(f"Titles data file not found: {titles_file}")
        print(f"Titles data file not found: {titles_file}")
...
def extract_text_from_content(content_data):
    ...
    print(f"Extracted {len(text_chunks)} text chunks from content data")
    return text_chunks

def create_tfidf_vectorizer(text_chunks):
    """Create a TF-IDF vectorizer for text chunks."""
...
def create_faiss_index(embeddings):
    ...
    if embeddings is None:
        ...
        print("No embeddings to create FAISS index for")
        return None
...
def create_vector_database():
    ...
    if not text_chunks:
        ...
        print("No text chunks extracted. Cannot create vector database.")
        return
    
    # Create TF-IDF vectorizer
    vectorizer = create_tfidf_vectorizer(text_chunks)
    
    # Create embeddings
    embeddings = create_embeddings(text_chunks, vectorizer)
    
    if embeddings is None:
        logger.error("Failed to create embeddings. Cannot create vector database.")
        print("Failed to create embeddings. Cannot create vector database.")
        return
    ...
    if index is None:
        ...
        print("Failed to create FAISS index. Cannot create vector database.")
        return
    ...
    # Save vectorizer
    vectorizer_file = os.path.join(OUTPUT_DIR, f"vectorizer_{timestamp}.pkl")
    ...
    latest_vectorizer_file = os.path.join(OUTPUT_DIR, "latest_vectorizer.pkl")
    with open(latest_vectorizer_file, 'wb') as f:
        pickle.dump(vectorizer, f)
    ...
    metadata = {
        ...
        "files": {
            ...
            "latest_vectorizer": latest_vectorizer_file,
    ...
    print(f"Saved vector database metadata to: {metadata_file}")
    ...
    print(f"Successfully created vector database with {len(text_chunks)} chunks")
...
def test_vector_search(query, top_k=5):
    ...
    # Load the vectorizer
    vectorizer_file = metadata["files"]["latest_vectorizer"]
    ...
    if metadata:
        ...
        if results:
            ...
            for i, result in enumerate(results):
                ...
                print(f"Title: {result['title_name']}")
